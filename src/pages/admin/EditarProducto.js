...
const EditarProducto = () => {
    ...
    const Formulario = () => {
        ...
        const update = async (e) => {
            ...
            const data = { descripcion: descripcion, nombre: nombre, estado: estado, foto: foto, precio:precio, categoria:categoria, recomendado:recomendado, favorito: favorito};
        ...
        const getProductoById = async (id) => {
            ...
            if(productos.exists()) {
                ...
                setEstado(productos.data().estado)
                setFoto(productos.data().foto)
                setPrecio(productos.data().precio)
                setCategoria(productos.data().categoria)
                setRecomendado(productos.data().recomendado)
                setFavorito(productos.data().favorito)
            }else{
                Swal.fire({
                    position: 'top-center',
                    icon: 'error',
        ...
        return(
            <>
                <Typography align="left" variant="h4" gutterBottom >Modificar {nombre}</Typography>
                
                <div className="mb-3 col-12" style={{ backgroundImage:`url(${foto})`, backgroundPosition:'center', backgroundSize:'cover', height:250}}></div>

                <form className="row g-3 text-start needs-validation" onSubmit={update}>
                  <div className="mb-3 col-6">
                    <label htmlFor="formFile" className="form-label" >Editar foto </label>
